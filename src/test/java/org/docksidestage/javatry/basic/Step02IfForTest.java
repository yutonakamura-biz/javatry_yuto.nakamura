...
public class Step02IfForTest extends PlainTestCase {
    ...
    public void test_if_elseif_nested() {
        ...
        // done jflute 1on1にて、漠然読みフォロー予定 (2026/08/16)
    ...
    // done nakamura [いいね] 違いをしっかりまとめられてGood by jflute (2026/08/16)
    // そして、「変更を加えないなら安全？」という発想もGood。
    // done jflute 1on1にて、forEach()メソッドの仕組みの話 (2026/08/16)
    // done jflute 1on1にて、"なんでもできること" と "適材適所" のジレンマの話 (2026/08/16)

    // #1on1: forEach()メソッドの仕組み (2026/09/15)
    //
    // forEach()メソッドの中で、外側のローカル変数の再代入ができない理由は？
    // $関数の中に、渡されていない変数は使えないものだから!?
    // -> {} はオブジェクト、つまり、(test_メソッドから見れば)別クラス別メソッド。
    // forEach()メソッドのソースコードを読むと、そのオブジェクトを受け取って...
    // 拡張for文を使っているだけ。ループの代理人見たいなメソッド。
    // 単なるメソッド。Javaの特別な文法というわけではなく、普通のメソッド。
    //
    // もし再代入ができちゃったら、ローカル変数のコンセプトが崩れる。
    // 時系列的な矛盾も発生して、カオスを生み出しやすくなっちゃう。
    // なので、できないようにしている。(ローカル変数はあくまでローカルスコープ)
    //
    // 参照は、(実質的な)final変数であればできる。副作用も発生しにくい。
    // mutableな変数は参照もできない。これはこれでカオスを生みやすい。
    //
    // break/continueが使えない理由もこれでしっくり。

    // #1on1: じゃあ、forEach()メソッドの存在意義は？ (2026/09/15)
    // $それ質問したかった
    // intあいのfor文: Java当初から (1995年)
    // 拡張for文: Java10年目くらいから (2005年くらい)
    // forEach()メソッド: Java20年目くらいから (2015年くらい)
    //
    // 制約の多いループがforEach()。
    // $変更を加えないならforEachを使ったほうが安全？
    // yes
    // immutable/mutableを思い出してみよう。
    // できないことに意義がある。
    // 工業デザインではできないから使いやすいにもなる。
    // 物理の世界の安全と、論理の世界の安全は、見やすさが違う。
    // こういう物の見方大事。
    //
    // webサービスのプログラムだったら、ストレートなループの方が多い。
    // なので、forEach()メソッドが向いている場面が多い。
    //
    // $forEach()メソッドの実現方法が美しい、実装がシンプルで演出がしっかりできてる
    // 適材適所スタイルになっている。
    //
    // よもやま: 適材適所過ぎるのもつらい (2026/09/15)
    // 使い分けの判断コスト。
    // 統一の世話ないメリットを優先するか？適材適所を優先するか？
    // ぼくらのジレンマと戦っている。
    // 
    // なので、Javaの拡張for文とforEach()メソッドの適材適所が正解とは限らない。
    //
    // $設計思想というものに触れることができた
